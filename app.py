...
def webhook():
    data = request.json
    print(data) # Render logs වල මැසේජ් එක පෙන්නුම් කරයි
    
    try:
        # WhatsApp එකෙන් එන මැසේජ් එක අල්ලාගැනීම
        if "entry" in data:
                ...
                for change in entry.get("changes", []):
                    ...
                    if "messages" in value:
                        ...
                        for message in messages:
                            from_number = message["from"] # මැසේජ් එක එවපු කෙනාගේ නම්බර් එක
                            msg_body = message["text"]["body"] # මැසේජ් එකේ වචන
                            
                            print(f"Received message from {from_number}: {msg_body}")
                            
                            # උට ආපහු රිප්ලයි එකක් යැවීම (උදාහරණයක් ලෙස)
                            send_whatsapp_message(from_number, f"Botee gen reply ekak: {msg_body}")
    except Exception as e:
