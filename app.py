...
def get_ai_reply(user_message: str) -> str:
    if not GROQ_API_KEY:
        return "AI chat disabled. Server GROQ_API_KEY environment variable set කරන්න."
        ...
        if not state["in_position"]:
            if signal == "BUY":
                if is_duplicate_candle:
                    messages.append(f"ℹ️ Duplicate Candle Protection: Timestamp {candle_ts} already processed.")
                elif in_cooldown:
...
def secure_webhook(secret_token):
    ...
    try:
        if "messageData" in data:
            ...
            is_trading_cmd = any(text_lower == t or text_lower.startswith(t + " ") for t in trading_triggers)
